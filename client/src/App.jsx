...
const statuses = ["Pendiente de contacto", "En validación", "Validado", "No contactado", "Disponible para recojo", "Entregado"];
...
function CaseForm({ winner, patch, errors, save, saving, openAttempt }) {
  ...
  return <div className="case-form"><div className="case-hero"><div className="winner-identity"><Avatar label={initials(winner.full_name)} size="xlarge" shape="circle" /><div><p className="eyebrow">CASO SELECCIONADO</p><h2>{winner.full_name}</h2><span>DNI {winner.dni}</span></div></div><Tag value={winner.delivery_status} severity={severityFor(winner.delivery_status)} /></div><div className="attempt-summary"><div><strong>Intentos de contacto</strong><span>{attemptCount} registrado{attemptCount === 1 ? "" : "s"}</span></div></div>
    ...
    <div className="case-controls"><div className="form-actions"><Button label="Registrar intento" icon="pi pi-phone" outlined disabled={saving} onClick={openAttempt} /><Button label="Guardar cambios" icon="pi pi-save" loading={saving} disabled={saving} onClick={save} /></div></div><Divider />
